...
| `virustotal_api_key` | `""` (mati) | isi API key → tiap file dicek via hash SHA256 ke VirusTotal (yang keluar hanya hash, bukan isi file) |
| `virustotal_fail_closed` | `false` | `true` = API VT error ikut menggagalkan (default: fail-open + catatan, agar VT down tak menumbangkan upload) |
